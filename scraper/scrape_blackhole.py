...
Blackhole Coffee Roaster(blackholecoffee.jp、東京都荒川区町屋)の商品情報を
...
実データ確認済み: 全12商品中、product_type="コーヒー"の11件がコーヒー豆。
残り1件は"ギフト"でコーヒー豆単品ではないため対象外。
...
SHOP_INFO = {
    "name": "Blackhole Coffee Roaster",
    "url": "https://blackholecoffee.jp/",
    "platform": "Shopify",
    "address": "東京都荒川区町屋4-31-11",
    "prefecture": "東京都",
    "robots_txt_status": "実質許可(2026-09確認。Shopify標準のrobots.txtでAllow: /、制限なし)",
}

PRODUCTS_JSON_URL = "https://blackholecoffee.jp/products.json?limit=250"
...
def build_record(product: dict) -> dict | None:
    ...
    product_url = f"https://blackholecoffee.jp/products/{product.get('handle')}"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        with open("data_blackhole.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_blackhole.json に出力しました"
