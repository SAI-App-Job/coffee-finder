...
TONE UP COFFEE(toneupcoffee.base.shop、茨城県取手市東、自家焙煎豆の
オンライン販売、小規模ロースター)の商品情報を取得する。BASE(白ラベル
ドメイン)。
...
【非コーヒー豆商品の除外について】
実データ確認済み(全151件): カタログの大半(約120件)が「ドリップバッグ
○○」というイラスト・メッセージ入りの個別デザインバリエーション
(猫・犬・季節の挨拶等、豆自体は共通)で、豆単品ではない。また
「紅茶」(2件)・「ラッピング」資材(3件)・「Cold Brew（水出し
アイスコーヒー）用バッグ」・「しらぬいカフェオレベース」(濃縮液)・
「アソートセット」「保護猫応援セット」等の詰め合わせ・「【定期便】」
...
SHOP_INFO = {
    "name": "TONE UP COFFEE",
    "url": "https://toneupcoffee.base.shop/",
    "platform": "BASE",
    "address": "茨城県取手市東6-37-7只石ビル102",
    "prefecture": "茨城県",
...
BASE_URL = "https://toneupcoffee.base.shop"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_toneup.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_toneup.json に出力しました"
