...
ORIGIN_COUNTRY_KEYWORDS = {
    ...
    # 誤ってインド産と判定してしまう)。
    "台湾": "台湾",  # 実データ確認済み(いつか珈琲屋、「台湾　花音農園（近藤select）」)。
    # 日本語の地名・単語と衝突するリスクが無いため、中国("雲南"のような
    # ローカルフォールバックが必要だった国)とは異なりここに直接追加できる。
    ...
    "マレーシア": "マレーシア",  # 実データ確認済み(HIMONYA FIVE COFFEE、「リベリカ プレミアム
    # タイガー」の原産国欄)。他の地名・単語と衝突するリスクが無い。
    "カメルーン": "カメルーン",  # 実データ確認済み(HIMONYA FIVE COFFEE、「ンベサ村のピーベリー」の
    # 原産国欄)。他の地名・単語と衝突するリスクが無い。
    "ハワイ": "アメリカ(ハワイ)",  # 実データ確認済み(HIMONYA FIVE COFFEE、原産国欄の表記
    # 「アメリカ（ハワイ）」より)。REGION_TO_COUNTRYの「コナ」と同じ正規化先に揃えている。
    "ハイチ": "ハイチ",  # 実データ確認済み(豆香洞コーヒー、「ハイチ　マールブランシュ」)。
    # 英語表記("haiti")もORIGIN_COUNTRY_KEYWORDS_ENに無く、日英とも未登録だった。
    # 他の地名・単語と衝突するリスクが無いため直接追加できる。
    "ラオス": "ラオス",  # 実データ確認済み(Vin COFFEE、「ラオス ルアンパバーン ウォッシュド」)。
    # 他の地名・単語と衝突するリスクが無いため直接追加できる。
    "ミャンマー": "ミャンマー",  # 実データ確認済み(又吉コーヒー園、「【ミャンマー産】自家焙煎
    # コーヒー豆」)。英語表記("myanmar")はORIGIN_COUNTRY_KEYWORDS_ENに既にあったが、
    # 日本語カタカナ表記がJP側の辞書に無く未検出だった(ブルンジ・ウガンダと同種の抜け)。
}
...
ORIGIN_COUNTRY_KEYWORDS_EN = {
    ...
    "bolivia": "ボリビア", "peru": "ペルー",
    "perú": "ペルー",  # 実データ確認済み(COFFEE COUNTY Kurume、"Perú"とアクセント記号付き表記)
    "ecuador": "エクアドル", "sudan": "スーダン",
    "thailand": "タイ", "dr congo": "コンゴ民主共和国", "congo": "コンゴ民主共和国",
    "dominica": "ドミニカ国",
    "india": "インド",  # 実データ確認済み(TSUKIKOYA COFFEE ROASTER、"INDIA Balmaadi"表記)
    "haiti": "ハイチ",  # 実データ確認済み(豆香洞コーヒー)
    "taiwan": "台湾",  # 実データ確認済み(いつか珈琲屋)
    "myanmar": "ミャンマー",  # 実データ確認済み(沖縄セラードコーヒー、"MYANMAR/GREENLAND SL-28"表記)
}
...
REGION_TO_COUNTRY = {
    ...
    "イルガチェフ": "エチオピア",  # 実データ確認済み(幸音珈琲、「モカ　イルガチェフ」。末尾の
    # 「ェ」が無い表記ゆれ)。
...
# 「フルシティ」は「シティ」を部分文字列として含むため、必ず長い方を先に
# 判定する(実データ確認済み: Muiの「あまにがいブレンド（フルシティロースト）」
# 等がkw探索順の影響で誤って「シティロースト」と判定されるバグが実際に
# 発生していた。GRADE_PATTERNの「G-1→G1正規化」コメントやkunikuni.pyの
# ROAST_GROUP_LABELS(「フルシティロースト」を「シティロースト」より先に
# 判定)と同じ「長い表記を先に」の原則)。
ROAST_KEYWORDS = {
    "ライト": "ライトロースト", "シナモン": "シナモンロースト",
    "ミディアム": "ミディアムロースト", "ハイ": "ハイロースト",
    "フルシティ": "フルシティロースト", "シティ": "シティロースト",
...
BLEND_KEYWORDS = ["ブレンド", "blend", "ミックス"]  # 実データ確認済み(こやまこおふぃ、
# 「新ハイマンミックス」「こやまミックス」。「ブレンド」の同義語として使われている)
