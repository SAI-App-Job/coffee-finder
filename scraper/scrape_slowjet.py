...
SLOW JET COFFEE(slowjetcoffe.official.ec、東京都足立区千住東)の商品情報を
取得する。BASEの白ラベルドメイン「.official.ec」。情報サイト
(slowjetcoffee.com、WordPress、注文機能無し)とは別にこちらが実際の
オンラインショップ。
...
【非コーヒー豆商品の除外について】
実データ確認済み(sitemap.xml上15件): トートバッグ・エプロン・キャップ・
Tシャツ・タンブラー・キャニスター(密閉缶)等のグッズ、ドリップバッグが
コーヒー豆単品ではないためNON_BEAN_KEYWORDSで除外する。残りはブレンド
3種+ストレート4種の計7件(2026-09-21再確認時点)。

【flavor_notes(2026-09-21追記)】
実データ確認済み: og:descriptionに対象7件全てでテイスティング文が入って
おり(ブレンドは物語文、ストレート豆は【国・地域】【フレーバー】等の
ラベル付きスペック)、注文/配送案内等の無関係な定型文の混入は無いため
全文をそのまま採用する。
"""
...
SHOP_INFO = {
    "name": "SLOW JET COFFEE",
    "url": "https://slowjetcoffe.official.ec/",
    "platform": "BASE",
    "address": "東京都足立区千住東1丁目",
    "prefecture": "東京都",
...
BASE_URL = "https://slowjetcoffe.official.ec"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = [
    "TOTE", "エプロン", "cap", "CAP", "T-SHIRTS", "タンブラー", "ドリップバッグ", "パネル",
    "キャニスター",
]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_slowjet.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_slowjet.json に出力しました"
