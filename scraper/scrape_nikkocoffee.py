...
実データ確認済み: og:descriptionにテイスティング文・産地説明が直接
入っており(対象13件全て確認)、末尾に必ず「【<ロースト度>】」という
焙煎度合いの表記が付いている(商品名にも同じ表記があり重複するため
除外する)。末尾の【】表記を取り除いた部分を採用する。
"""
...
TRAILING_ROAST_PATTERN = re.compile(r"【[^】]*】\s*$")
...
def extract_flavor_notes(description: str | None) -> str | None:
    ...
    if not description:
        return None
    text = TRAILING_ROAST_PATTERN.sub("", description)
