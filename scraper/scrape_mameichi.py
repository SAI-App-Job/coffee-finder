...
珈琲工房豆壱(mameichi.thebase.in、千葉県柏市中央2-9-11-102、フジローヤル
直火式5kg焙煎機使用)の商品情報を取得する。BASEの白ラベルドメイン
「.thebase.in」。情報サイト(mameichi-coffee.com、静的HTML、注文機能無し)
とは別にこちらが実際のオンラインショップ。
...
【非コーヒー豆商品の除外について】
実データ確認済み(sitemap.xml上59件): 「定期便」(サブスクリプション)、
水だしコーヒー(液体)、ダージリンティー・紅茶(複数)、アイスコーヒー
リキッド、カフェオレベース、CAFECブランドの器具(フィルター・ミル・
サーバー)、コーヒードリップバッグ/ドリップバック(複数)、「お試し
...
SHOP_INFO = {
    "name": "珈琲工房豆壱",
    "url": "https://mameichi.thebase.in/",
    "platform": "BASE",
    "address": "千葉県柏市中央2-9-11-102",
    "prefecture": "千葉県",
...
BASE_URL = "https://mameichi.thebase.in"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = [
    "定期便", "水だし", "ダージリン", "紅茶", "リキッド", "カフェオレベース",
    "CAFEC", "ドリップバッグ", "ドリップバック", "セット",
]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_mameichi.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_mameichi.json に出力しました"
