...
自家焙煎 珈琲豆店 柊豆(Tozu coffee、tozucoffee.base.ec、埼玉県熊谷市
船木、自家焙煎豆のオンライン販売)の商品情報を取得する。BASE(白ラベル
ドメイン)。
...
【非コーヒー豆商品の除外について】
実データ確認済み(全11件): 「ﾛｰｽﾀｰおまかせ三種セット」「浅～深煎り
おまかせ４種セット」の2件が複数銘柄の詰め合わせセットのため
NON_BEAN_KEYWORDSで除外する。残り9件はいずれもシングルオリジン
(100g/200g固定、重量バリエーション無し)。

【flavor_notes(2026-09-21追記)】
実データ確認済み: og:descriptionに対象9件全てでテイスティング文・産地
背景が入っており(生産国/地域/農園/樹種/精選/標高等のスペック情報が
地続きで混在するがfull-text-tolerance方針により許容)、注文/配送案内等の
...
BASE_URL = "https://tozucoffee.base.ec"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_tozu.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_tozu.json に出力しました"
