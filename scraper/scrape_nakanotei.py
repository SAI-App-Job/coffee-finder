...
NAKANOTEI COFFEE(nakanotei.thebase.in、京都府京都市下京区、おばんざい・
日本酒の店「なかの邸」が「暮らしランプ」ブランドで展開する自家焙煎豆の
オンライン販売)の商品情報を取得する。BASE。

事前確認事項: 主業態は「なかの邸」というおばんざい・日本酒の飲食店で、
コーヒー焙煎は副業的な位置づけだが、実データ確認済み(2026-09時点)で
珈琲豆100g/200gが6銘柄(西山ブレンド・インドネシア・タイ・イエメン・
ラオス・デカフェ)×2サイズ=12件、常設の商品ラインとしてオンライン
ショップに掲載されていることを確認した。季節限定・単発ではなく継続的な
販売品目と判断し実装対象とする。
...
【重量違いの重複について】
実データ確認済み: 全16件中、珈琲豆の6銘柄が100g/200gの2サイズで個別
商品登録されている。商品名末尾の「100g」「200g」表記を除いた基準名で
...
SHOP_INFO = {
    "name": "NAKANOTEI COFFEE",
    "url": "https://nakanotei.thebase.in/",
    "platform": "BASE",
    "address": "京都府長岡京市調子1丁目6-35 中野家住宅",
    "prefecture": "京都府",
...
BASE_URL = "https://nakanotei.thebase.in"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    with open("data_nakanotei.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_nakanotei.json に出力しました"
