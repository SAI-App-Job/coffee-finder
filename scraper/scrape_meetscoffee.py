# -*- coding: utf-8 -*-
"""
scrape_meetscoffee.py

ミーツコーヒーロースターズ(meetscoffee.theshop.jp、岐阜県、自家焙煎豆の
...
【重量違いの重複について】
実データ確認済み: 全21件中、主力の10銘柄(ストレート7種+ブレンド3種)が
100g/200gの2サイズで個別商品登録されている。商品名末尾の重量を除いた
基準名でグルーピングし、最小重量(100g)を代表として採用する。

【非コーヒー豆商品の除外について】
実データ確認済み: 水出しアイスコーヒーバッグ(【500cc用×4袋】、粉を
充填したパック商品で単一銘柄の豆売りではない)が非対象。
NON_BEAN_KEYWORDSで除外する。残り20件(10銘柄×2サイズ)を対象とする。

【flavor_notes(2026-09-21追記)】
実データ確認済み: og:descriptionに対象10件全てでテイスティング文・産地
背景が入っており(ストレート豆は地域/品種/標高/精選のスペック情報が
末尾に地続きで混在するがfull-text-tolerance方針により許容)、注文/配送
...
SHOP_INFO = {
    "name": "ミーツコーヒーロースターズ",
    "url": "https://meetscoffee.com/",
    "platform": "BASE",
    "address": "岐阜県岐阜市西河渡3-78-1",
    "prefecture": "岐阜県",
...
BASE_URL = "https://meetscoffee.theshop.jp"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    with open("data_meetscoffee.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_meetscoffee.json に出力しました"
