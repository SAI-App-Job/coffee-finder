# -*- coding: utf-8 -*-
"""
scrape_thekokubocoffee.py

the;kokubo(thekokubocoffee.com、山梨県甲府市国母4-21-10、自家焙煎豆の
...
【非コーヒー豆商品の除外について】
実データ確認済み: 全11商品にproduct_typeが不揃い(空欄／「焙煎豆」／
「ラテベース」／「コーヒー」)のため、商品名でのキーワード除外方式を
使う。Latte base(ラテベース、液体)・珈琲便り(店主の近況を綴った
読み物と思われ、産地・重量の記載が無い)が非対象。
NON_BEAN_KEYWORDSで除外する。残り9件(国名のみのシンプルな商品名)を
対象とする。

【重量バリエーションについて】
実データ確認済み: 一部銘柄は100g/200gの2バリアントを持ち、それ以外は
Default Titleの単一バリアント(重量表記なし)。variants配列のgramsから
最小重量を代表として採用する。
"""
...
SHOP_INFO = {
    "name": "the;kokubo",
    "url": "https://thekokubocoffee.com/",
    "platform": "Shopify",
    "address": "山梨県甲府市国母4-21-10",
    "prefecture": "山梨県",
    "robots_txt_status": "実質許可(2026-09確認。Shopify標準のrobots.txtでAllow: /、制限なし)",
}

PRODUCTS_JSON_URL = "https://thekokubocoffee.com/products.json?limit=250"
CRAWL_DELAY_SECONDS = 1.0
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = ["Latte base", "珈琲便り"]
...
def build_record(product: dict) -> dict | None:
    ...
    product_url = f"https://thekokubocoffee.com/products/{product.get('handle')}"
...
if __name__ == "__main__":
    ...
    with open("data_thekokubocoffee.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_thekokubocoffee.json に出力しました"
