...
奥久慈珈琲焙煎所ルージュノワール(rougenoir.base.shop、茨城県久慈郡
大子町袋田、自家焙煎豆のオンライン販売)の商品情報を取得する。BASE
(白ラベルドメイン)。
...
【非コーヒー豆商品の除外について】
実データ確認済み(全37件): 各銘柄に対応する「【ドリップバッグ】○○」
(12件、豆単品と重複する銘柄名だが挽き済みドリップバッグ)・「水出し
アイスコーヒー」(液体)・HARIO/三洋産業の器具(7件)がコーヒー豆単品では
...
SHOP_INFO = {
    "name": "奥久慈珈琲焙煎所ルージュノワール",
    "url": "https://rougenoir.base.shop/",
    "platform": "BASE",
    "address": "茨城県久慈郡大子町袋田一條2978-1",
    "prefecture": "茨城県",
...
BASE_URL = "https://rougenoir.base.shop"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_rougenoir.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_rougenoir.json に出力しました"
