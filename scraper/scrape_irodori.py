...
こーひー豆のお店いろどりこーひー(irodori-coffee.jp、東京都江戸川区中葛西)の
...
実データ確認済み: 全62商品中、product_type="コーヒー豆"の35件が対象。
「贈答セット」(6件)・「コーヒーバッグ」(5件)・「メッセージカード、
熨斗等」(16件)は対象外。「コーヒー豆」タクソノミー内にも
「ブレンド3点セット」のような複数銘柄の詰め合わせが1件含まれるため
...
SHOP_INFO = {
    "name": "いろどりこーひー",
    "url": "https://irodori-coffee.jp/",
    "platform": "Shopify",
    "address": "東京都江戸川区中葛西1-38-8",
    "prefecture": "東京都",
    "robots_txt_status": "実質許可(2026-09確認。Shopify標準のrobots.txtでAllow: /、制限なし)",
}

PRODUCTS_JSON_URL = "https://irodori-coffee.jp/products.json?limit=250"
...
def build_record(product: dict) -> dict | None:
    ...
    product_url = f"https://irodori-coffee.jp/products/{product.get('handle')}"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        with open("data_irodori.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_irodori.json に出力しました"
