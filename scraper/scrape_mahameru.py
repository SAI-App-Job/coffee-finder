...
MAHAMERU COFFEE(www.mahameru.tokyo、千葉県、自家焙煎豆のオンライン
販売、小規模カタログ)の商品情報を取得する。Shopify(/products.json
全件取得方式)。
...
PRODUCTS_JSON_URL = "https://www.mahameru.tokyo/products.json?limit=250"
...
def build_record(product: dict) -> dict | None:
    ...
    product_url = f"https://www.mahameru.tokyo/products/{product.get('handle')}"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        with open("data_mahameru.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_mahameru.json に出力しました"
