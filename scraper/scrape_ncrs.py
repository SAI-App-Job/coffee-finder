...
ナカムラコーヒーロースターズ(ncrs.theshop.jp、新潟県長岡市与板町与板乙
3620-43、自家焙煎豆のオンライン販売)の商品情報を取得する。
...
【重量違いの重複について】
実データ確認済み: 大半の銘柄が100g/250g/500gの3サイズ(一部は100g/250g
のみ)で個別商品登録されている。商品名から末尾の重量を除いた基準名で
グルーピングし、最小重量(100g)を代表として採用する。

【非コーヒー豆商品の除外について】
実データ確認済み: ドリップバッグ/コーヒーバッグ(単品・セット)・
ギフトセット・手提げ紙袋が非対象。NON_BEAN_KEYWORDSで除外する。

【flavor_notes(2026-09-21追記)】
実データ確認済み: og:descriptionの先頭に簡潔なテイスティング文が入って
おり(対象13件全て確認)、必ず「【<ロースト度>】」という焙煎度合いの
表記が続き、その後に販売期間の案内・生産国/農園等のスペック情報・
挽き具合や発送方法に関する店舗共通の定型注意書きが続く。最初の
「【」の手前までを採用する。
"""
...
SHOP_INFO = {
    "name": "ナカムラコーヒーロースターズ",
    "url": "https://ncrs.theshop.jp/",
    "platform": "BASE",
    "address": "新潟県長岡市与板町与板乙3620-43",
    "prefecture": "新潟県",
...
BASE_URL = "https://ncrs.theshop.jp"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = ["ドリップバッグ", "コーヒーバッグ", "ギフトセット", "手提げ紙袋"]
...
def pick_canonical_items(items: list[dict]) -> list[dict]:
    ...
    for item in items:
        base_name = re.sub(r"[\s　]+", " ", WEIGHT_PATTERN.sub("", item["title"])).strip()
...
if __name__ == "__main__":
    ...
    with open("data_ncrs.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_ncrs.json に出力しました"
