# -*- coding: utf-8 -*-
"""
scrape_juunintomame.py

十人十豆(kasamacoffee.base.shop、茨城県笠間市笠間、レストラン
「Monomi」内でポップアップ形式で営業する自家焙煎豆のオンライン販売)の
...
【非コーヒー豆商品の除外について】
実データ確認済み(全16件): 「ドリップバック」を含む商品(通常版・
定期便・ギフト、11件)がコーヒー豆単品ではないためNON_BEAN_KEYWORDSで
除外する。残り5件は「【ブレンドコーヒー】TOMAMEブレンド」1件+
「【受注焙煎：...】」の4件(いずれも「生豆100g」という単位表記だが、
「受注焙煎」(注文を受けてから焙煎する)という商品説明から実際には
焙煎済み豆が届く方式と判断し、焙煎前の重量をweight_gとして採用する
(珈琲豆のおおつか・熊谷珈琲の「生豆時」表記と同じ扱い)。
...
SHOP_INFO = {
    "name": "十人十豆",
    "url": "https://kasamacoffee.base.shop/",
    "platform": "BASE",
    "address": "茨城県笠間市笠間2517-1",
    "prefecture": "茨城県",
...
BASE_URL = "https://kasamacoffee.base.shop"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_juunintomame.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_juunintomame.json に出力しました"
