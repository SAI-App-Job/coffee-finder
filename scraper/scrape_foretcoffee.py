# -*- coding: utf-8 -*-
"""
scrape_foretcoffee.py

Foret coffee(foretcoffee.thebase.in、長野県上水内郡信濃町大字穂波1495、
...
【重量違いの重複について】
実データ確認済み: 全18件が6銘柄(産地+焙煎度の組み合わせ)×150g/300g/
500gの3サイズで構成されている(同じ産地でも浅焼き/深焼きは別銘柄として
扱う。例: ETHIOPIA NATURALには浅焼き版と深焼き版が存在)。商品名から
"『重量』"の部分を除いた基準名でグルーピングし、最小重量(150g)を
代表として採用する。

【flavor_notes(2026-09-21追記)】
実データ確認済み: og:descriptionに対象6件全てでテイスティング文+産地
スペック(TASTE/COUNTRY/LOCATION/PROCESSING/ALTITUDE/VARIETAL)が入って
...
BASE_URL = "https://foretcoffee.thebase.in"
REQUEST_HEADERS = {
...
def pick_canonical_items(items: list[dict]) -> list[dict]:
    ...
    for item in items:
        base_name = re.sub(r"\s+", " ", WEIGHT_TOKEN_PATTERN.sub("", item["title"])).strip()
...
if __name__ == "__main__":
    ...
    with open("data_foretcoffee.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_foretcoffee.json に出力しました"
