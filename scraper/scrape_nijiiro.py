...
にじいろコーヒー店(nijiirocafe.handcrafted.jp、東京都葛飾区水元)の
商品情報を取得する。BASEのカスタムドメイン(handcrafted.jp、
nano-coffeeroaster等と同系列)。
...
【非コーヒー豆商品の除外について】
実データ確認済み(sitemap.xml上24件): 同一銘柄が「（豆）」「（粉）」
「ドリップバッグ」の3形態で重複掲載されている(挽いた粉・ドリップ
...
SHOP_INFO = {
    "name": "にじいろコーヒー店",
    "url": "https://nijiirocafe.handcrafted.jp/",
    "platform": "BASE",
    "address": "東京都葛飾区水元",
    "prefecture": "東京都",
...
BASE_URL = "https://nijiirocafe.handcrafted.jp"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = ["（粉）", "(粉)", "ドリップバッグ", "お試しセット"]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_nijiiro.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_nijiiro.json に出力しました"
