# -*- coding: utf-8 -*-
"""
scrape_eaglecoffee.py

自家焙煎珈琲　イーグルコーヒー高山(eagle-coffee.shop-pro.jp、岐阜県高山市
石浦町2丁目398番地、有限会社イーグルコーヒー高山、自家焙煎豆のオンライン
...
ボットを個別にDisallow: /、それ以外は制限なし。sitemap.xmlはhttp
スキームのままだと301でhttpsへリダイレクトされるため、本スクレイパーは
最初からhttps://eagle-coffee.shop-pro.jpを起点に使う。

【非コーヒー豆商品の除外について】
実データ確認済み: 全22件中、リッチドリップパック30個入り(単品・ギフト
各1件、ドリップパック=器具的な加工品で豆売りではない)・自家焙煎コーヒー
...
SHOP_INFO = {
    "name": "イーグルコーヒー高山",
    "url": "https://eagle-coffee.com/",
    "platform": "カラーミーショップ",
    "address": "岐阜県高山市石浦町2丁目398番地",
    "prefecture": "岐阜県",
...
BASE_URL = "https://eagle-coffee.shop-pro.jp"
REQUEST_HEADERS = {
...
def fetch_pid_urls() -> list[str]:
    ...
    return [
        loc.get_text(strip=True).replace("http://", "https://")
        for loc in soup.find_all("loc") if "pid=" in loc.get_text()
    ]
...
if __name__ == "__main__":
    ...
    with open("data_eaglecoffee.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_eaglecoffee.json に出力しました"
