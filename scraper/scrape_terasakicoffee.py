# -*- coding: utf-8 -*-
"""
scrape_terasakicoffee.py

TERASAKI COFFEE 寺崎COFFEE(terasakicoffee.com、山梨県甲府市丸の内
1-20-22、自家焙煎豆のオンライン販売)の商品情報を取得する。
...
実データ確認済み: 全9商品中、product_type="コーヒー豆"の8件が対象。
ただし「テイスティングセット Mini」はこのタグが付いているものの
アソート(試飲用詰め合わせ)のためNON_BEAN_KEYWORDSで別途除外する。
「Lemon Cake Gift Box」(product_type空欄)は非対象。
...
SHOP_INFO = {
    "name": "TERASAKI COFFEE",
    "url": "https://terasakicoffee.com/",
    "platform": "Shopify",
    "address": "山梨県甲府市丸の内1-20-22",
    "prefecture": "山梨県",
    "robots_txt_status": "実質許可(2026-09確認。Shopify標準のrobots.txtでAllow: /、制限なし)",
}

PRODUCTS_JSON_URL = "https://terasakicoffee.com/products.json?limit=250"
...
def build_record(product: dict) -> dict | None:
    ...
    product_url = f"https://terasakicoffee.com/products/{product.get('handle')}"
...
if __name__ == "__main__":
    ...
    with open("data_terasakicoffee.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_terasakicoffee.json に出力しました"
