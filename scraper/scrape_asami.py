...
実データ確認済み: 詳細ページのdiv.item_desc_text内に実際のテイス
ティング文が入っている(対象20件全てで確認)。末尾に「◆焙煎と味わい◆」
「◆味覚バランス◆」「【香味バランス】」「【味覚バランス】」等の
星取り表付きラベルが続く場合はその直前までを採用する(ラベルが無い
商品は全文を採用)。
"""
...
FLAVOR_STOP_PATTERN = re.compile(r"[◆【].*バランス[◆】]")
