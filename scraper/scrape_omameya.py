...
御豆屋(shop.omameya.jp、東京都江戸川区、JR小岩駅北口)の商品情報を
取得する。BASE。
...
【非コーヒー豆商品の除外について】
実データ確認済み(sitemap.xml上114件): ドリップバッグ、水出し珈琲パック、
チャフ茶(コーヒーの果肉から作るお茶、コーヒー豆ではない)がコーヒー豆
...
SHOP_INFO = {
    "name": "御豆屋",
    "url": "https://shop.omameya.jp/",
    "platform": "BASE",
    "address": "東京都江戸川区(小岩駅北口)",
    "prefecture": "東京都",
...
BASE_URL = "https://shop.omameya.jp"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = ["ドリップバッグ", "水出し", "チャフ茶"]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_omameya.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_omameya.json に出力しました"
