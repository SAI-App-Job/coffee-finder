# -*- coding: utf-8 -*-
"""
scrape_saitobeans.py

自家焙煎珈琲豆 彩豆(shop.saito-beans.com、秋田県由利本荘市出戸町字赤沼下道
72-17、自家焙煎豆のオンライン販売)の商品情報を取得する。BASE(白ラベル
ドメイン)。
...
【住所について】
特定商取引法ページ(https://shop.saito-beans.com/law)で実データ確認済み
(2026-09時点): 「由利本荘市出戸町字赤沼下道72-17」との記載を確認。候補
リストの住所と一致。
...
【重量違いの重複について】
実データ確認済み(sitemap.xml全32件): 大半の銘柄が「100g」「200g」「500g」の
複数形態で個別商品登録されている(バリアントではない)。商品名中の
"(数字)g"セグメントを除去した基準名でグルーピングし、最小重量を代表として
採用する(298coffeeと同じ方式)。

【非コーヒー豆商品の除外について】
実データ確認済み: ギフト詰め合わせ(3件)・お試しセット・オーナーズセレクト
(複数銘柄から選ぶ福袋的商品、豆/粉の2件)・ドリップバッグ(3件)が非対象。
NON_BEAN_KEYWORDSで除外する。

【flavor_notes(2026-09-21追記)】
実データ確認済み: og:descriptionにテイスティング文が直接入っている
(対象12件全て確認)。一部商品では「当店のネットショップはすべて、
注文を受けてから、焙煎したものを発送いたします。」という店舗共通の
注意書きが(装飾記号「ー」付きの場合もある)テイスティング文の途中に
挿入されているため、これを除去する。また末尾に必ず「共通価格」で
始まる重量別価格表が続く商品もあるため、その手前までを採用する。
"""
...
SHOP_INFO = {
    "name": "自家焙煎珈琲豆 彩豆",
    "url": "https://shop.saito-beans.com/",
    "platform": "BASE",
    "address": "秋田県由利本荘市出戸町字赤沼下道72-17",
    "prefecture": "秋田県",
...
BASE_URL = "https://shop.saito-beans.com"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = ["ギフト", "セット", "オーナーズセレクト", "お試し", "ドリップバッグ"]
WEIGHT_PATTERN = re.compile(r"(\d+)\s*[gｇ]")
WEIGHT_SEGMENT_PATTERN = re.compile(r"[（(]\s*\d+\s*[gｇ]\s*[）)]")
NOTICE_PATTERN = re.compile(r"ー?当店のネットショップはすべて、?注文を受けてから、?焙煎したものを発送いたします。ー?")
PRICE_STOP_PATTERN = re.compile(r"共通価格")
TRAILING_DASH_PATTERN = re.compile(r"[ー\s]+$")
...
def extract_flavor_notes(description: str | None) -> str | None:
    ...
    if not description:
        return None
    text = NOTICE_PATTERN.sub("", description)
    m = PRICE_STOP_PATTERN.search(text)
    if m:
        text = text[: m.start()]
...
if __name__ == "__main__":
    ...
    with open("data_saitobeans.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_saitobeans.json に出力しました"
