# -*- coding: utf-8 -*-
"""
scrape_renshirocoffee.py

恋史郎コーヒー(www.renshirocoffee.com、〒880-0805 宮崎県宮崎市橘通東3-3-8
カブトビル102、自家焙煎豆のオンライン販売)の商品情報を取得する。BASE。

【ドメインについて】
実データ確認済み(2026-09時点): 候補リストの`renshirocoffee.com`(wwwなし)は
DNS未解決で接続できない。`www.renshirocoffee.com`(wwwあり)のみ稼働している。
...
【住所について】
特定商取引法ページ(https://www.renshirocoffee.com/law)で実データ確認済み
(2026-09時点): 事業者名「田中友太」、所在地「〒8800805 宮崎県宮崎市橘通東
3-3-8カブトビル102」との記載を確認。候補リストの住所と一致。

【商品情報の取得方法について】
実データ確認済み: 他のBASE系店舗と同様、SNSシェア用OGPメタタグ
(`og:title`・`product:price:amount`)から商品名・価格を取得する。
sitemap.xmlの/items/配下に全24件。

【対象商品について】
実データ確認済み(全24件): 水出しコーヒーパック(2件)・ドリップバッグ単品/
ギフトボックス(2件)・コーヒー豆付きのグラインダー/ケトル(2件)・
マグ/グラス/コーヒーサーバー等の器具(3件)・PRANA CHAI(チャイスパイス
ブレンドで対象外、1件)を除外すると、単一銘柄のコーヒー豆7銘柄
(エチオピア・ルワンダ・ホンジュラス・コロンビア・メキシコデカフェ・
ミドルブレンド・ダークブレンド)が残り、いずれも2種類の重量違いで
登録されている。NON_BEAN_KEYWORDSで除外する。

【重量違いの重複について】
実データ確認済み: 全7銘柄とも2種類の重量(100/150g、200/300g)が個別商品
...
SHOP_INFO = {
    "name": "恋史郎コーヒー",
    "url": "https://www.renshirocoffee.com/",
    "platform": "BASE",
    "address": "宮崎県宮崎市橘通東3-3-8カブトビル102",
    "prefecture": "宮崎県",
...
BASE_URL = "https://www.renshirocoffee.com"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = [
    "水出し", "Drip Bag", "ギフトボックス", "グラインダー", "MUG", "GLASS",
    "CHAI", "サーバー", "ケトル",
]
...
if __name__ == "__main__":
    ...
    with open("data_renshirocoffee.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_renshirocoffee.json に出力しました"
