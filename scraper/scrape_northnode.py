...
northnodecoffee(northnode.base.shop、東京都足立区保木間)の商品情報を
取得する。BASEの白ラベルドメイン「.base.shop」。
...
【商品情報の取得方法について】
実データ確認済み: 他のBASE系店舗と同様、SNSシェア用OGP(Open Graph)
メタタグ(`og:title`・`product:price:amount`)から商品名・価格を取得する
(GitHub Actions環境でGTMのdataLayerスクリプトが条件付きで除去される
不具合が過去に見つかっているため、OGPメタタグを標準採用している)。

【配送方法違いによる重複掲載について】
実データ確認済み(sitemap.xml上36件): 全銘柄が「【ネコポス】」
...
SHOP_INFO = {
    "name": "northnodecoffee",
    "url": "https://northnode.base.shop/",
    "platform": "BASE",
    "address": "東京都足立区保木間3-15-14",
    "prefecture": "東京都",
...
BASE_URL = "https://northnode.base.shop"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_northnode.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_northnode.json に出力しました"
