...
KiaOra COFFEE(shop.kiaoracoffee.com、埼玉県春日部市大沼、自家焙煎豆の
...
【非コーヒー豆商品の除外について】
実データ確認済み(全15件): 「コーヒーチェリーティー『カスカラ』」
(コーヒーの果実の皮のお茶、豆ではない)・「ドリップバッグ シングル5種」
「ドリップバッグ デカフェ5個」がコーヒー豆単品ではないため
NON_BEAN_KEYWORDSで除外する。残り12件はシングルオリジン9件+ブレンド
2件+デカフェ1件、いずれも200g固定で重量バリエーションは無い。

【flavor_notes(2026-09-21追記)】
実データ確認済み: og:descriptionにテイスティング文・産地背景が地続きで
入っており(対象12件全て確認)、その後に「プロセス」または「生産国」
(商品によりどちらが先に出現するかは不定)で始まるスペック欄、続けて
「コーヒー器具と挽豆の選び方」という店舗共通の挽き方ガイドが入る。
これらのうち最初に出現するものの手前までを採用する。
"""
...
SHOP_INFO = {
    "name": "KiaOra COFFEE",
    "url": "https://shop.kiaoracoffee.com/",
    "platform": "BASE",
    "address": "埼玉県春日部市大沼3-123-1",
    "prefecture": "埼玉県",
...
BASE_URL = "https://shop.kiaoracoffee.com"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = ["カスカラ", "ドリップバッグ"]
WEIGHT_PATTERN = re.compile(r"(\d+)\s*[gｇ]")
FLAVOR_STOP_PATTERN = re.compile(r"プロセス|生産国|コーヒー器具と挽豆の選び方")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_kiaora.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_kiaora.json に出力しました"
