...
パロットコーヒー(parrot-coffee.com、群馬県前橋市、自家焙煎豆のオンライン
販売)の商品情報を取得する。カラーミーショップ。
...
SHOP_INFO = {
    "name": "パロットコーヒー",
    "url": "https://www.parrot-coffee.com/",
    "platform": "カラーミーショップ",
    "address": "群馬県前橋市",
    "prefecture": "群馬県",
...
BASE_URL = "https://www.parrot-coffee.com"
REQUEST_HEADERS = {
...
def fetch_pid_urls() -> list[str]:
    ...
    for loc in soup.find_all("loc"):
        m = re.search(r"gid=(\d+)", loc.get_text(strip=True))
        if m:
            gids.add(m.group(1))
    ...
    for gid in gids:
        ...
        for a in cat_soup.select('a[href*="pid="]'):
            ...
            if m:
                pids.add(m.group(1))
    return [f"{BASE_URL}/?pid={pid}" for pid in pids]
...
if __name__ == "__main__":
    ...
    with open("data_parrot.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_parrot.json に出力しました"
