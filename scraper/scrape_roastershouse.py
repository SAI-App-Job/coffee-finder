# -*- coding: utf-8 -*-
"""
scrape_roastershouse.py

ROASTER's HOUSE(www.roasters-house.com、千葉県千葉市花見川区花園1-20-7、
注文後焙煎)の商品情報を取得する。BASEのカスタムドメイン。「www.」無しの
裸ドメインはDNS解決不可のため、必ず「www.」付きでアクセスする。
...
実データ確認済み: 「ROASTER's HOUSEロゴマグカップ」(陶器製マグカップ、
豆単品ではない)が既存のNON_BEAN_KEYWORDSに引っかからず除外漏れしていた。
今回のflavor_notes実装でマグカップの商品説明(重量・容量等)を誤って
テイスティング文として取得してしまうため、「マグカップ」をNON_BEAN_
KEYWORDSに追加して除外した。

【flavor_notes(2026-09-20追記)】
実データ確認済み: og:descriptionの先頭に実際のテイスティング・商品紹介文
があり、その後に「■地域：」「■標高：」「■おすすめの焙煎度合い：」等の
構造化スペックラベルが続く。最初の「■」の直前までをflavor_notesとして
採用する。
"""
...
SHOP_INFO = {
    "name": "ROASTER's HOUSE",
    "url": "https://www.roasters-house.com/",
    "platform": "BASE",
    "address": "千葉県千葉市花見川区花園1-20-7",
    "prefecture": "千葉県",
    "robots_txt_status": "実質許可(2026-09確認。他のBASE系店舗と同一の記述。"
                          "curl/python-requests等は個別にDisallow: /指定あり、"
                          "本スクレイパーは識別可能なUser-Agentを使用。"
                          "「www.」無しドメインはDNS不可のためwww.roasters-house.comを使用)",
}

BASE_URL = "https://www.roasters-house.com"
REQUEST_HEADERS = {
    "User-Agent": "CoffeeFinderBot/0.1 (+contact: your-contact-info-here)"
}

NON_BEAN_KEYWORDS = ["ドリップバッグ", "ドリップバック", "ギフト", "セット", "詰め合わせ", "マグカップ"]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_roastershouse.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_roastershouse.json に出力しました"
