...
COFFEE GALLERY(cgconline.official.ec、埼玉県川越市松江町、自家焙煎豆の
オンライン販売)の商品情報を取得する。BASE(official.ecカスタム
ドメイン)。
...
【非コーヒー豆商品の除外について】
実データ確認済み(全19件): 「ロースターズセレクション...２種セット」
(詰め合わせ)・「コーヒー豆 定期便」(豆/粉の2種、サブスクリプション)・
「ドリップバッグ」・「ラテベース」「リキッドコーヒー」(いずれも
...
SHOP_INFO = {
    "name": "COFFEE GALLERY",
    "url": "https://cgconline.official.ec/",
    "platform": "BASE",
    "address": "埼玉県川越市松江町2-3-5",
    "prefecture": "埼玉県",
...
BASE_URL = "https://cgconline.official.ec"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_cgconline.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_cgconline.json に出力しました"
