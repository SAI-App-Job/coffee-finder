...
珈琲自家焙煎HiwaHiwa(hiwahiwaweb.thebase.in、東京都江戸川区中葛西、
社会福祉法人江戸川菜の花の会が運営する就労支援事業所)の商品情報を
取得する。BASEの白ラベルドメイン「.thebase.in」。
...
【非コーヒー豆商品の除外について】
実データ確認済み(sitemap.xml上20件): 商品は動物の名前を冠したブレンド
(「パンダブレンド」＝マイルド、「ゴリラブラジル」等)で、大半が
お試しセット・ギフトボックス・プチギフト・ドリップパック(すべて
ギフト/加工品向けの詰め合わせ・個包装)。単一銘柄そのものを指す
...
SHOP_INFO = {
    "name": "珈琲自家焙煎HiwaHiwa",
    "url": "https://hiwahiwaweb.thebase.in/",
    "platform": "BASE",
    "address": "東京都江戸川区中葛西2-7-2",
    "prefecture": "東京都",
...
BASE_URL = "https://hiwahiwaweb.thebase.in"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        import json
        with open("data_hiwahiwa.json", "w", encoding="utf-8") as f:
            json.dump(output, f, ensure_ascii=False, indent=2)
        print(f"[done] {len(records)}件を data_hiwahiwa.json に出力しました"
