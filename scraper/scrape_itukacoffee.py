...
(id=2239064)と「シングルオリジン／産地別」(id=2239068)の2カテゴリで豆売り
商品を網羅できる。「焙煎度合いで選ぶ」配下の4小カテゴリは上記2つと同じ商品を
焙煎度別に再掲載しているだけの導線(実データ確認済み: 同一商品URLが複数
カテゴリに出現)のため、二重取得を避けるためスクレイピング対象に含めない。

【カテゴリ内混入への対応について】実データ確認済み(2026-08時点、初回実行後に
data/products.jsonを確認して発覚): 「オリジナルブレンド」カテゴリには豆売り
以外に「【飲み比べ】ブレンド100ｇ×３種セット」(比較セット)・「【希釈用】
牛乳と仲の良いコーヒー（カフェオレベース）」(カフェオレベース、本来
「カフェオレベース」専用カテゴリ(id=4294892)の商品)・「【コーヒーバッグ】
Top of Japan　10個」(コーヒーバッグ、本来「コーヒーバッグ」専用カテゴリ
(id=2571587)の商品)が混在していた。「飲み比べセット」「コーヒーバッグ」
「カフェオレベース」「ギフト」「福袋」の各専用カテゴリを直接除外するだけでは
防げない(同じ商品が複数カテゴリに重複掲載されているため)ため、商品名に
「セット」「カフェオレベース」「コーヒーバッグ」のいずれかを含む商品を
除外する。
...
EXCLUDE_TITLE_KEYWORDS = ["セット", "カフェオレベース", "コーヒーバッグ"]
