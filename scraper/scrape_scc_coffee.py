# -*- coding: utf-8 -*-
"""
scrape_scc_coffee.py

珈琲工房SCC(scc-coffee.com、滋賀県彦根市後三条町613-8、1994年創業の
自家焙煎豆専門店)の商品情報を取得する。カラーミーショップ(shop-pro.jp、
scc-coffee.shop-pro.jp)。

自家焙煎確認済み: 商品ページog:description「もちろん全て自家焙煎です。」。
...
【非コーヒー豆商品の除外について】
実データ確認済み(sitemap.xml掲載の全40件): 以下が非対象。
  - 「箱（200g×2個用）」「箱（200g×3個用）」「箱（200g×4個用）」
    「ラッピング（200g×1個）」の4件: 化粧箱・ラッピングサービスで
    コーヒー豆そのものではない
  - 「生豆　工房ブレンド:1kg」「生豆　スペシャルブレンド:1kg」
    「生豆　モカブレンド:1kg」の3件: 商品詳細ページに「こちらの珈琲豆は、
    生豆となっております。焙煎豆ではありませんので、ご注意ください。」と
    明記されており(専用の「生豆」カテゴリに分類、1kg単位販売、焙煎度・
    グラインドの選択オプションも無し)、家庭焙煎用の未焙煎豆販売と確認
...
SHOP_INFO = {
    "name": "珈琲工房SCC",
    "url": "https://scc-coffee.com/",
    "platform": "カラーミーショップ",
    "address": "滋賀県彦根市後三条町613-8",
    "prefecture": "滋賀県",
...
BASE_URL = "https://scc-coffee.shop-pro.jp"
REQUEST_HEADERS = {
...
if __name__ == "__main__":
    ...
    with open("data_scc_coffee.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    print(f"[done] {len(records)}件を data_scc_coffee.json に出力しました"
