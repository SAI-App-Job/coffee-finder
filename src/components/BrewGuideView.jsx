...
export function BrewMethodCard({ brew }) {
  return (
    <div className="rounded-2xl bg-[#2F241A] border border-[#4A3A2A] p-4 flex flex-col gap-3">
      <div>
        <p className="text-[11px] tracking-[0.15em] text-[#8B5E2E] uppercase">{brew.english}</p>
        <h3 className="font-serif text-[19px] text-[#F2E9DD] mt-0.5">{brew.method}</h3>
        <p className="text-[13px] text-[#D4A24E] mt-1">{brew.tagline}</p>
      </div>

      <div className="grid grid-cols-3 gap-2">
        <div className="bg-[#3B2211] rounded-lg p-2 flex flex-col items-center gap-1 text-center">
          <Coffee size={13} className="text-[#8B5E2E]" strokeWidth={1.75} />
          <span className="text-[11px] text-[#B8A891] leading-tight">{brew.ratio}</span>
        </div>
        <div className="bg-[#3B2211] rounded-lg p-2 flex flex-col items-center gap-1 text-center">
          <Thermometer size={13} className="text-[#8B5E2E]" strokeWidth={1.75} />
          <span className="text-[11px] text-[#B8A891] leading-tight">{brew.temp}</span>
        </div>
        <div className="bg-[#3B2211] rounded-lg p-2 flex flex-col items-center gap-1 text-center">
          <Timer size={13} className="text-[#8B5E2E]" strokeWidth={1.75} />
      ...
      {brew.grindSize && (
        <p className="text-[12px] text-[#B8A891]">
          <span className="text-[#8B5E2E]">粒度:</span> {brew.grindSize}
        </p>
      )}
      ...
      <ol className="flex flex-col gap-2">
        {brew.steps.map((step, i) => (
          <li key={i} className="flex gap-2.5 text-[13px] text-[#B8A891] leading-relaxed">
            <span className="shrink-0 w-5 h-5 rounded-full bg-[#3B2211] border border-[#4A3A2A] text-[#D4A24E] text-[11px] flex items-center justify-center font-mono">
              {i + 1}
            </span>
            {step}
          </li>
      ...
      {brew.calibration && (
        <div className="flex items-start gap-1.5 text-[12px] text-[#8B7361] border-t border-[#4A3A2A] pt-2.5">
          <SlidersHorizontal size={13} className="shrink-0 mt-0.5 text-[#8B5E2E]" strokeWidth={1.75} />
...
export function ExtractionVariableCard({ v }) {
  return (
    <div className="rounded-xl bg-[#3B2211] border border-[#4A3A2A] p-3">
      ...
      <p className="text-[12px] text-[#D4A24E] mt-0.5">{v.summary}</p>
...
export function ExtractionScienceSection() {
  return (
    <div className="rounded-2xl bg-[#2F241A] border border-[#4A3A2A] p-4 flex flex-col gap-3">
      <div>
        <p className="text-[11px] tracking-[0.15em] text-[#8B5E2E] uppercase">Extraction Science</p>
      ...
      <div>
        <p className="text-[11px] tracking-wide text-[#8B5E2E] uppercase mb-2">未抽出・適正・過抽出の見分け方</p>
