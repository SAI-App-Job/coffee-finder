          ...
          return (
            <button
              ...
              className={`text-left rounded-2xl bg-[#2F241A] border border-[#4A3A2A] p-4 flex flex-col gap-1.5 transition-colors ${
                matchedOrigin ? "hover:border-[var(--accent-label)]" : "opacity-70 cursor-default"
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <p className="font-serif text-[16px] text-[#F2E9DD]">{b.name}</p>
                <span className="shrink-0 text-[11px] px-2 py-0.5 rounded-full bg-[#3B2211] text-[var(--accent-muted)] border border-[#4A3A2A]">
                  {b.country ?? "キューバ"}
                </span>
              </div>
              ...
              {matchedOrigin ? (
                <span className="text-[11px] text-[var(--accent)] mt-1">産地タブでこの国を見る →</span>
              ) : (
